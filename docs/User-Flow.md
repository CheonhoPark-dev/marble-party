...
A[진행자: 방 생성] --> B[QR 공유]
B --> C{참여자 입장 성공?}
C -- 아니오 --> C1[접속 문제 안내/수동 코드 입력]
C1 --> C
C -- 예 --> D[FEAT-0: 규칙 요약/준비 완료]
D --> E{모두 입장했나?}
E -- 아니오 --> E1[대기 타이머/재초대]
E1 --> E
E -- 예 --> F[FEAT-2: 룰렛/레이싱 설정]
F --> G[시작]
G --> H[FEAT-3: 버튼 활성화]
H --> I[공 하강/장애물 조작]
I --> J{공정성 불만 발생?}
J -- 예 --> J1[즉시 재설정/재시작]
J -- 아니오 --> K[당첨자 발표/축하 연출]
K --> L[간단 피드백]
L --> M[리텐션: 다음 행사 리마인더/보상]
M --> A
```

## 흐름 메모
- MVP는 A~E까지 집중(입장/대기실).
- F~K는 이후 확장 범위로 설계만 유지.
