...
# Coding Convention & AI Collaboration Guide — 마블 파티
...
## 핵심 원칙
- “신뢰하되, 검증하라(Don't trust, verify)”
- 최종 책임은 사람에게 있다(실행 전 리뷰 필수)

## 프로젝트 설정/기술 스택 권장
- 대중적인 웹 스택 선호(유지보수/채용 용이)
- 버전관리 필수, 작은 단위 커밋 권장

## 아키텍처/모듈성
- 진행자 화면과 참여자 화면을 분리 모듈로 구성
- 실시간 상태는 “세션 관리 모듈”로 단일화
- 공통 UI/카피는 shared 레이어로 관리

## AI 소통 원칙
- 하나의 채팅 = 하나의 작업 범위
- 요구사항, 제약, 참고 문서를 먼저 요약하고 시작
- 기존 코드 재사용 우선, 새로운 도입은 이유 명시
